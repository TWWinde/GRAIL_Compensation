...
def main():
    ...
    if mode == "pruning":
        ...
        print("\n=== Evaluation AFTER compression (before Compensation REPAIR) ===")
        pruned_params = count_parameters(pruned_model)
        acc_after = test(pruned_model, val_loader, device=DEVICE)
        print(f"🔹 Top-1 Accuracy: {acc_after:.2f}%")

        # Apply compensation step
        compensator = ResNet18_PruneCompensation(pruned_model, ridge_lambda=1e-3)
        compensator.load_compression_state(prune_entries)
        ...
        repair_bn(compensated_model, train_loader)

        # Evaluate after folding
        print("\n=== Evaluation AFTER compression (after Compensation REPAIR) ===")
        acc_after = test(compensated_model, val_loader, device=DEVICE)
        compensated_params = count_parameters(compensated_model)
        print(f"🔹 Top-1 Accuracy: {acc_after:.2f}%")
        print(f"Pruned Parameters: {compensated_params}")
        print(f"🔥 Compression Ratio: {(original_params - compensated_params) / original_params:.2%}")
    else:
        folder = ResNet18_ModelFolding(model, compression_ratio=COMPRESSION_RATIO)
        folder.run_calibration(train_loader, DEVICE, num_batches=50)
        folded_model = folder.apply()
        folded_model_ = copy.deepcopy(folded_model)
        ...
        # Evaluate after compression and repair without compensation
        print("\n=== Evaluation AFTER REPAIR (w/o compensation) ===")
        repair_bn(folded_model_, train_loader)
        acc_after = test(folded_model_, val_loader, device=DEVICE)
        print(f"🔹 Top-1 Accuracy: {acc_after:.2f}%")
      


        # Apply compensation step
        comp = ResNet18_FoldingCompensation(folded_model, ridge_lambda=1e-3)
        comp.load_fold_state(fold_stats)
        comp.load_gram_stats(gram_stats)
        compensated_model = comp.apply()
